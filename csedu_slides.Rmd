...
1. [Used Model: Rregression Discounity Design](#RDD)
...
- __Research Idea:__ students should receive objective and motivating feedback through an  warning email   
...
- Information of __802__ individuals were collected
...
- We used two data sources 
  - First three online test results  
  - Cumulative points of the tasks in JACK
- Logit model was used to predict students probability to pass the final exam based on the first 3 online tests
  - Model was trained with the latest data obtained from previous edition of the same course
...
- 57 days between the warning mail and 1st exam 
- 113 days between the warning email and 2nd exam
...
.font80[
- `r TextCite(my_bib, 'Arnold2012')` investigated the effect of the signal light system at the Purdue University and found a positive effect on grades
- `r TextCite(my_bib, 'baneres2020')` implemented an early warning system but did not analysed the effect on students' performance
- `r TextCite(my_bib, 'csahin2019')` invented an _Intelligent Intervention System_ where for each assessment the students get feedback. Students emphasized the usefulness of the system. 
- `r TextCite(my_bib, 'Iver2019')` could not find an effect from their early waning system in the ninth grad.
...
- Many studies analyzed the effect based on questionnaires
]
...
<br>
- We also estimated the RDD with covariates 
...
- The bandwidth of $0.255$ was determined with the data-driven approach of `r Citet(my_bib, 'imbensoptimal2009')`.
  - Only students with a predicted probability $0.4$ (cutoff) $\pm 0.255$ (bandwidth), are included in the analysis.
- This leads to the sample size of $126$ students.
...
- The variance around the cutoff is rather high, which compromises the detection of an effect 
- Many individuals are not included in the final analysis for several reasons
  - Students dropping the course
  - Students far away from the cutoff are not providing much information for the model

-> Thus precise estimation of the treatment becomes more difficult


---
# Discussion &#8212; II

- Students get also feedback through their online tests
- The warning might may also lead weak students to postpone participation to a later semester
- The motivation and objective feedback from one warning email is rather small
...
- An automatic repeatedly feedback system could possibly have a greater impact on students motivation
  - A detialed recurring feedback could also used to guide students
...
- Consider the following _sharp_ RDD representation `r Citet(my_bib, "Huntington-Klein2022")`: 
...
<ul>
  ...
  <li>Two estimation approaches: parametric and non-parametric</li>
  <ul>
